...
| **비보호 (Not protected)** | 로그인 세션 쿠키, QR이 아닌 일반 페이지 | 인증된 사이트 로그인 중에만 운영자에게 노출 |
| **위협 행위자** | 단말 앞에 앉은 비인가 직원, 어깨너머 관찰자, 화면 공유/원격 데스크톱 | 운영 통제 수준에서 억제 |
| **명시적 비대상** | 로컬 관리자, 리버스 엔지니어, 디스크 직접 접근자 | 막지 못함을 문서화 |

**Fail-safe 방향(설계 불변식):** 앱이 "지금 QR이 보이면 안 되는 상태인지"에 대해 확신이 없으면, **항상 잠금 쪽으로 행동한다.** 노출은 오직 올바른 지역 인증 또는 관리자 인증으로 시작한 사이트 로그인 흐름에서만 일어난다.
...
- 첫 실행 설정: QR URL, 관리자 인증 코드, 사용자 1명 이상(아이디+코드), 노출 시간(기본 10초), 유휴 자동잠금 시간.
- 설정 화면(관리자 인증 필요): QR URL, 노출 시간, 사용자 추가/수정/삭제, 사용자 코드 재설정, QR 화면 제목, QR 세션 초기화.
- 잠금 해제 흐름:
...
- 인증 없이 QR 사이트 페이지를 노출하는 자동 URL/title 판정 금지.
...
```
states: needsSetup | locked | unlocked | siteLogin | settings
...
* QR 노출(setVisible true)은 unlocked 또는 siteLogin일 때만.
```

**QR 노출 가시성 규칙(단일 함수로 강제):**
```
function shouldShowQrView(state):
    if state == 'unlocked': return true
    if state == 'siteLogin': return true
    return false   // locked, settings, needsSetup, unknown → 숨김
```
...
```
- v1/v2/v3 설정은 현재 스키마로 마이그레이션하며, 과거의 자동 로그인 규칙은 로드 시 버린다.
...
window.qrGuard = {
  ...
  submitSiteLogin(adminCode): { ok, errorCode? }         // 관리자 인증 후 사이트 로그인
  manualLock()
  learnCurrentQrTitle()
...
- **Wave 3:** 관리자 사이트 로그인 + QR 제목 감지, 감사 로그/내보내기, 듀얼 패키징 + 최종 QA.
...
- [ ] **4. 첫 실행 설정 + 설정 UI**
  할 일 / 금지: 첫 실행 시 QR URL, 관리자 코드, 사용자 1명+, 노출 시간(기본 10s), 유휴 자동잠금 시간 입력. 설정 진입은 관리자 인증 필요. QR URL/사용자 CRUD/코드 재설정/노출·유휴 시간/QR 화면 제목/QR 세션 초기화. raw hash·내부 경로 노출 금지.
...
- [ ] **5. 상태 머신 + 잠금 타이머 + 노출 흐름**
  할 일 / 금지: 상태 `needsSetup|locked|unlocked|siteLogin|settings`. **가시성은 `shouldShowQrView()` 단일 함수로만 제어.** 올바른 인증 → `unlockDurationSeconds` 노출 후 **reload 없이** relock. 실패 코드는 잠금 유지+성공 로그 없음. 성공 시 감사 이벤트+`lastAuthenticatedAt` 갱신. 잠금 시 QR webContents/쿠키 파괴 금지.
...
- [ ] **8. 듀얼 패키징(macOS + Windows) + 운영 문서 + 최종 QA**
  ...
  QA: Happy — 패키지 앱이 첫설정→잠금해제→relock→siteLogin→설정 여정 완료. Failure — QR 세션 초기화 후 관리자 인증 전 QR 사이트가 숨김 유지.
